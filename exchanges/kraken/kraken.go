...
func (k *Kraken) SendAuthenticatedHTTPRequest(method string, params url.Values, result interface{}) (err error) {
	...
	signature := common.Base64Encode(common.GetHMAC(common.HashSHA512, append([]byte(path), shasum...), []byte(k.API.Credentials.Secret)))

	if k.Verbose {
		log.Debugf("Sending POST request to %s, path: %s, params: %s", k.API.Endpoints.URL, path, encoded)
	}
